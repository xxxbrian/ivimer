...
ge                  # 向后移动到单词词尾
gE                  # 向后移动到空白隔开的单词词尾
gg                  # 跳到第一行，等效于 Ctrl+<Home>
G                   # 跳到最后一行，等效于 Ctrl+<End>
...
:[N]sv[iew] {file}  # 横向切分窗口并在新窗口打开文件 file，等效于 :split，区别在于当前窗口的内容 只读
...
Ctrl+W c            # 关闭当前窗口，但不能关闭最后一个窗口，等效于 :clo[se]
Ctrl+W o            # 关闭其他窗口，只保留当前活动窗口，等效于 :on[ly]
Ctrl+W q            # 退出当前窗口，如果是最后一个窗口，则退出 Vim，等效于 :q[uit]
...
:tabr[ewind]        # 切换到第一个标签页，等效于 :tabfirst 命令
:tabfir[st]         # 切换到第一个标签页
:tabl[ast]          # 切换到最后一个标签页
:tabn[ext]          # 切换到下一个标签页，等效于 gt
:tabp[revious]      # 切换到上一个标签页，等效于 gT
gt                  # 切换到下一个标签页
gT                  # 切换到上一个标签页
...
:setl[ocal]                  # 等效于 :set，但对局部选项设定其局部值
:setg[lobal]                 # 等效于 :set，但对局部选项设定其全局值
...
:lw[indow] [height] # 等效于 :cwindow，除了使用显示当前窗口的位置列表
:cbo[ttom]          # 将光标放在 Quickfix 窗口的最后一行并滚动以使其可见
:lbo[ttom]          # 等效于 :cbottom，除了使用显示当前位置列表的窗口
...
![vim 命令键盘图](./image/vim-keys_CN.png)
